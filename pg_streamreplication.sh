...
function configure_master {
    ...
    for ((i = 0; i < ${#slave_hosts[@]}; i++)); do
        ...
        sudo -u $user | ssh $user@${slave_hosts[$i]} 'ssh-keyscan '${master_hosts[$host_n]}' >> $HOME/.ssh/known_hosts'
    done
...
function configure_slave {
    ...
    sudo -u $user cat `su $user -c 'echo $HOME'`/id_rsa | ssh $user@$slave_host 'mkdir -p $HOME/.ssh && cat >> $HOME/.ssh/id_rsa && chmod 600 $HOME/.ssh/id_rsa && chown postgres:postgres $HOME/.ssh/id_rsa'
    printf "Copy public rsa key from $slave_host to ${master_hosts[$host_n]}\n"
    su $user -c 'mkdir -p $HOME/.ssh'
    sudo -u $user | ssh $user@$slave_host 'ssh-keygen -q -t rsa -P "" -f $HOME/id_rsa && cat $HOME/id_rsa.pub >> $HOME/.ssh/authorized_keys && cat $HOME/id_rsa' | cat >> `su $user -c 'echo $HOME'`/.ssh/id_rsa
    chown postgres:postgres `su $user -c 'echo $HOME'`/.ssh -R
    chmod 600 `su $user -c 'echo $HOME'`/.ssh/id_rsa
...
su $user -c 'ssh-keygen -q -t rsa -P "" -f $HOME/id_rsa'
su $user -c 'mkdir -p $HOME/.ssh && cat $HOME/id_rsa.pub >> $HOME/.ssh/authorized_keys'
